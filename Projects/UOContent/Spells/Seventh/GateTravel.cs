...
public class GateTravelSpell : MagerySpell, IRecallSpell
{
    ...
    public void Effect(Point3D loc, Map map, bool checkMulti)
    {
        ...
        else if (CheckSequence())
        {
            ...
            var firstGate = new GateTravelMoongate(loc, map);
            firstGate.MoveToWorld(Caster.Location, Caster.Map);

            Effects.PlaySound(loc, map, 0x20E);

            var secondGate = new GateTravelMoongate(Caster.Location, Caster.Map);
...
public partial class GateTravelMoongate : Moongate
{
    public GateTravelMoongate(Point3D target, Map map) : base(target, map)
    {
        Map = map;
